...
def masterPlot(step, nodes, values, labels, xylim):
    ...
    for i in range(N):
        ax  = fig.add_axes([i*axwidth/figwidth,cbardepth/figdepth,axwidth/figwidth,axdepth/figdepth])
        ax.set_xlim(xylim[0,:])
        ax.set_ylim(xylim[1,:])
        cax = fig.add_axes([(i+0.1)*axwidth/figwidth,cbardepth/figdepth/2,0.8*axwidth/figwidth,cbardepth/figdepth/2])
