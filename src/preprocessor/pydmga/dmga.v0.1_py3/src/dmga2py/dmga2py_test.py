...
print("[TEST] summary volume = ", vol1 +
      vol2 + vol3 + vol4, "should be", 5*5*5)
...
while (dmga2py.cell_iterator_is_finished(iter) == 0):
    ...
    print("[TEST] arriving at", dmga2py.cell_iterator_current_vertex(
        iter), dmga2py.cell_iterator_current_vertex_coords(iter))
    if (dmga2py.cell_iterator_is_marked(iter) == 1):
        dmga2py.cell_iterator_jump(iter)
        print("[TEST] new side at", dmga2py.cell_iterator_current_vertex(
            iter), dmga2py.cell_iterator_current_vertex_coords(iter))
...
for dcel in dmgacells:
    ...
    while (dmga2py.cell_iterator_is_finished(iter) == 0):
        polygon.append(
            (dmga2py.cell_iterator_current_vertex(iter), {"alpha": 0.0}))
        dmga2py.cell_iterator_mark(iter)
        dmga2py.cell_iterator_forward(iter)
        if (dmga2py.cell_iterator_is_marked(iter) == 1):
