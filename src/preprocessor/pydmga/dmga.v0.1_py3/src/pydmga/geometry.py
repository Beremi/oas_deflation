...
class BoxGeometry(Geometry):
    ...
    def __init__(self, x_min, y_min, z_min, x_max, y_max, z_max, x_periodic=False, y_periodic=False, z_periodic=False):
        ...
        self._geometry_handle = dmga2py.new_orthogonal_geometry(
            x_min, y_min, z_min, x_max, y_max, z_max, x_periodic, y_periodic, z_periodic)
...
class OrthogonalGeometry(Geometry):
    '''
    ...
    def __init__(self, x_size, y_size, z_size, x_periodic=False, y_periodic=False, z_periodic=False):
        ...
        self.y_periodic = y_periodic
        self.z_periodic = z_periodic
        self.bounding_box = (0.0, 0.0, 0.0, x_size, y_size, z_size)
        self._geometry_handle = dmga2py.new_orthogonal_geometry(
            0, 0, 0, x_size, y_size, z_size, x_periodic, y_periodic, z_periodic)
...
class CastSphereGeometry(Geometry):
    ...
    def __init__(self, radius, origin=(0, 0, 0)):
        ...
        self._geometry_handle = dmga2py.new_orthogonal_geometry(origin[0] - self.radius,
                                                                ...
                                                                self.radius,
                                                                0, 0, 0)
        self._preset_handle = dmga2py.new_geometry_preset_sphere(
            origin[0], origin[1], origin[2], self.radius)
    ...
    def transform(self, id, x, y, z, r):
        ...
        '''
        dist = math.sqrt(
            (x - self.origin[0])**2 + (y - self.origin[1])**2 + (z - self.origin[2])**2)
        if (dist < 0.001):
            return (id, x, y, z, r)
# 		dist = dist + 0.02
...
# 			height_periodic = 0
        ...
        (x1, y1, z1, x2, y2, z2) = (-self.radius, -self.radius,
                                    0.0, self.radius, self.radius, self.height)
...
#  													vector[0], vector[1], vector[2])
        self._preset_handle = dmga2py.new_geometry_preset_approximate_cylinder(
            self.radius, self.height, precision)
...
class CastOrthoPlaneGeometry(OrthogonalGeometry):
    ...
    def __init__(self, from_geometry, vec):
        ...
        if (vec == (1, 0, 0)):
            ...
            raise Exception(
                "CastOrthoPlaneGeometry.__init__(): Bad cast vector!")
        ...
        OrthogonalGeometry.__init__(
            self, x_size, y_size, z_size, x_periodic, y_periodic, z_periodic)
        self.vector = vec
    ...
    def on_boundary(self, neighbour):
        ...
        '''
        return neighbour < 0 and neighbour % 2 != 0
