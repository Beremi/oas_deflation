...
class TrajectoryGenerator(ShowBase):
    ...
    def startOutput(self):
        if (not self.isOutputStarted):
            ...
            fileObj = open(os.path.join(self.repository,
                                        self.repository + ".desc"), "w")
            fileObj.write(
                " ".join(map(str, [2*self.rad, 2*self.rad, 2*self.rad, 0, 0, 0, "\n"])))
            i = 0
            for ball in self.balls:
                ...
                fileObj.write(
                    " ".join(map(str, [i, "A", "1", "LIP", x, y, z, 1.0])))
                fileObj.write("\n")
            fileObj.close()
    ...
    def doOutput(self):
        f_coord = open(os.path.join(self.repository, self.repository +
                                    ('%08d' % self.current_frame) + '.coord'), "w")
        for ball in self.balls:
            x = ball.getX() + self.rad
            y = ball.getY() + self.rad
            z = ball.getZ() + self.rad
            ...
            f_coord.write("\n")
        f_coord.close()
        f_voro = open(os.path.join(self.repository, self.repository +
                                   ('%08d' % self.current_frame) + '.voro'), "w")
        f_voro.close()
...
if __name__ == "__main__":
    ...
    app = TrajectoryGenerator(sys.argv[1], int(
