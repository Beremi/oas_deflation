...
class MultiFileInput(Input):
    ...
    def _make_current_path(self):
        self._coord_file = os.path.join(
            self._directory, self._basename, self._basename + ('%08d' % self._current_frame) + '.coord')
        self._voro_file = os.path.join(
            self._directory, self._basename, self._basename + ('%08d' % self._current_frame) + '.voro')
    ...
    def __init__(self, repository_path, start_frame=0, end_frame=None):
        ...
        self._description_file = os.path.join(
            self._directory, self._basename, self._basename + '.desc')
