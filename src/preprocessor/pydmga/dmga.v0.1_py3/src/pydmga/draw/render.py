...
from . import assets  # voro.graphics
...
class PointCloud(Primitive):
    ...
    def __init__(self):
        ...
        format_array.addColumn(InternalName.make(
            'vertex'), 3, Geom.NTFloat32, Geom.CPoint)
        format_array.addColumn(InternalName.make(
            'size'), 1, Geom.NTFloat32, Geom.COther)
        format_array.addColumn(InternalName.make(
            'color'), 4, Geom.NTFloat32, Geom.CColor)
...
class ConvexPolygons(Primitive):
    ...
    def add_side_point(self, number):
        ...
        if (self.base_point is None):
            ...
            self.triangles_primitive.addVertices(
                self.base_point, self.second_point, number)
            self.second_point = number
        return self
...
class OnSpherePrimitive(Primitive):
    ...
    def cast_to_sphere(self, vertex):
        # make a list of pairs suitable for computations with fancy python functions ;)
        v = [(vertex[i], self.origin[i]) for i in range(3)]
        # compute length of vector
        d = sqrt(sum(a for a in [(x[0]-x[1])**2 for x in v]))
        result = [(x[0]-x[1]) / d * self.radius + x[1] for x in v]
...
class SphereArcs(OnSpherePrimitive):
    ...
    def _generate_points_on_arc(self, center, from_v, to_v, circle_r=None):
        ...
        dist_sq = sum((from_v[i] - to_v[i]) **
                      2 for i in range(3))  # distance between points
        if circle_r is None:
    ...
    def _make_arc(self, center, from_v, to_v):
        ...
        for i in range(1, divisions+1):
            ...
            (x, y, z) = [c[i] + r * cos(theta) * a[i] +
                         r * sin(theta) * b[i] for i in range(3)]
            self.line_seqs.drawTo(Point3(x, y, z))
    ...
    def add_cutting_plane_arc(self, normal, distance, from_v, to_v):
        ...
        c = [self.origin[i] + n[i] * distance for i in range(3)]
...
class GreatArcs(OnSpherePrimitive):
    ...
    def _generate_points_on_arc(self, from_v, to_v, depth=0):
        ...
        dist_sq = sum((from_v[i] - to_v[i])**2 for i in range(3))
        if (dist_sq < (self.radius / 5.0)**2 or depth > 10):  # its quesss...
            # simply connects
            return [to_v]
        else:
            # generate middle point and call recursively
            new_v = self.cast_to_sphere(
                [(from_v[i] + to_v[i]) / 2.0 for i in range(3)])
...
class Render(ShowBase):
    ...
    def __init__(self, bgcolor=None, ltcolor=None):
        ...
        if bgcolor is None:
            bgcolor = (0.0, 0.0, 0.0, 1.0)  # default is black
        if ltcolor is None:
            ltcolor = (1.0, 1.0, 1.0, 1.0)  # default if white
        self.current_nodes = []
            ...
            if self.selected_cells[n]:
                ...
                for (x, y, z, point_data) in points:
                    ...
                    if (point_data['alpha'] <= alpha):
                        point_cloud.add_vertex(
                            (x, y, z), 10.0, (0.1, 1, 0.1, 0.75))
                ...
                for (poly, face_data) in faces:
                    ...
                    if (make_face):
                        ...
                        cell_polygon.add_side_point(
                            polygon_base_id + poly[0][0])

                    for i in range(1, len(poly)):
                        (x, y, z, point_data) = points[poly[i][0]]
                        if (make_face):
                            cell_polygon.add_side_point(
                                polygon_base_id + poly[i][0])
