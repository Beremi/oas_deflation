...
class CellEdgesIterator:
    ...
    **NOTE** this should not be created directly. Use cell.edges to get this iterator
    ...
    def __next__(self):
        ...
        edge = CellEdge(self._cell_handle, current, dmga2py.cell_iterator_current(
            self._iter), dmga2py.cell_iterator_inverse(self._iter))
        if self._as_coords:
...
class CellSideIterator:
    ...
    def __next__(self):
        ...
        '''
        index = next(self._vertices_iterator)
...
class CellSidesIterator:
    ...
    def __next__(self):
        ...
        neighbour = dmga2py.cell_iterator_current_side_neighbour_id(
            self._cell_handle, self._iter)
...
class Cell:
    ...
    def get_sides(self):
        ...
        while (dmga2py.cell_iterator_is_finished(iter) == 0):
            side.append(dmga2py.cell_iterator_current_vertex_coords(iter))
            neighbour = dmga2py.cell_iterator_current_side_id(
                self._cell_handle, iter)
            dmga2py.cell_iterator_mark(iter)
            dmga2py.cell_iterator_forward(iter)
            if (dmga2py.cell_iterator_is_marked(iter) == 1):
