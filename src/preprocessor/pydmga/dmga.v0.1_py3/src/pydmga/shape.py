...
class AlphaSimplex:
    ...
    **NOTE** This class should not be instantiated directly - you should use AlphaShapeCell iteration ability or accessors to get it  
    ...
    # 2-d simplex is edge as dually in Delaynay triangulation it vould be Triangle (side)
    DIM_EDGE = 2
    # 1-d simplex is edge as dually in Delaynay triangulation it vould be Edge connecting two particles (nearest neighbours)
    DIM_SIDE = 1
    ...
    def diagram_part(self):
        ...
        raise ValueError(
            "AlphaSimplex.diagram_part(): simplex with dimension " + str(self.dimension))
...
class AlphaShapeCellIterator:
    ...
    **NOTE** this class should not be instantiated directly - use AlphaCellShape iteration to get it.
    ...
    def __init__(self, alpha_shape_cell, alpha, start=0):
        ...
        self._i = start-1
        self._max = dmga2py.alpha_shape_complex_upper_bound(
            self._alpha_shape_cell._complex_handle, alpha)
    ...
    def __next__(self):
        ...
        if self._i >= self._max:
            raise StopIteration
        (dim, v, j, alpha, id, killed) = dmga2py.alpha_shape_complex_get_simplex(
            self._alpha_shape_cell._complex_handle, self._i)
...
class AlphaShape(Shape):
    ...
    **NOTE** if Container class is used to instantiate then the class will create Diagram for internal use
    ...
    def get_cell(self, number):
        ...
        '''
        (shape_cell_handle, cell_handle, complex_handle) = dmga2py.alpha_shape_get_shape_cell(
...
class SASAContoursIterator:
    ...
    def __next__(self):
        ...
        if (self._stop):
            raise StopIteration
        (first, on_plane, on_sphere,
         second) = dmga2py.sasa_contours_iterator_as_tuple_coords(self._iter_handle)
...
class SASAShape(Shape):
    ...
    **NOTE** if Container class is used to instantiate then the class will create Diagram for internal use    
    ...
    def get_cell(self, number):
        ...
        '''
        (shape_cell_handle, cell_handle, contour_handle) = dmga2py.sasa_shape_get_shape_cell(
            self._shape_handle, number)
