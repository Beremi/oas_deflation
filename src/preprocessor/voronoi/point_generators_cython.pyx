    ...
    while (tr < trials):
        ...
        while (not distIsGood) and (tr < trials):
            ...
            if (directionDim == 0 ):
                ...
                coords[1] = center[1] +radius * np.cos(angle) * rn
                coords[2] = center[2] +radius * np.sin(angle) * rn
            if (directionDim == 1):
                coords[0] = center[0] +radius * np.cos(angle) * rn
                coords[1] = center[1] +height * random.random()
                coords[2] = center[2] +radius * np.sin(angle) * rn
            if (directionDim == 2):
                coords[0] = center[0] +radius * np.cos(angle) * rn
                coords[1] = center[1] +radius * np.sin(angle) * rn
