...
void IndirectDC :: readFromStream(unsigned num, ifstream& inputfile) {
    ...
    while ( getline(inputfile >> std :: ws, line) ) {
        ...
        if (param.compare("idc_nodes") == 0 ) {
            ...
            for (unsigned j=0; j<num; j++) iss >> c_nodes[nummaxunit-1][j];
        } else if (param.compare("idc_xcoords") == 0 ) {
            coords_active[nummaxunit-1] = true;
            for (unsigned j=0; j<num; j++) iss >> xcoords[nummaxunit-1][j];
        } else if (param.compare("idc_ycoords") == 0 ) for (unsigned j=0; j<num; j++) iss >> ycoords[nummaxunit-1][j];
        else if (param.compare("idc_zcoords") == 0 ) for (unsigned j=0; j<num; j++) iss >> zcoords[nummaxunit-1][j];
        else if (param.compare("idc_directions") == 0 ) for (unsigned j=0; j<num; j++) iss >> c_dirs[nummaxunit-1][j];
...
void IndirectDC :: init(NodeContainer *nodes, FunctionContainer *funcs){
    unsigned clength;
    Node* n;
    if(funcnum>=0) func = funcs->giveFunction(funcnum);
