...
{
        if (length != v.size()) cerr << "Sparse vector size does not match for multiplication with Vector" << endl;
	double ret = 0 ;
...
{
       if (length != v.length) cerr << "Sparse vector sizes does not match for multiplication" << endl;
	double ret = 0 ;
	size_t i = 0 ; 
	size_t j = 0 ; 
...
bool ConjGrad(const CoordinateIndexedSparseMatrix &A, Vector &x, const Vector &b, const Vector x0) {
    ...
    if (bnorm<1E-30) {x = x*0.; return true;}
    ...
    Vector z(r);
    for(int i=0; i<b.size(); i++) z[i] = r[i]*preconditioner[i];
    Vector p = z;
    Vector q = A*p;
