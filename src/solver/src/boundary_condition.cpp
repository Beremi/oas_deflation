...
double PieceWiseLinearFunction::giveY(double t) const{
    ...
    else return y[i-1]+(y[i]-y[i-1])/(x[i]-x[i-1])*(t-x[i-1]);
}
...
FunctionContainer::~FunctionContainer(){
    for(vector<Function*>::iterator f=functions.begin(); f!=functions.end(); ++f) delete *f;    
}
...
void BoundaryCondition::init(){
    blockedDoFNum = 0;
    loadedDoFNum = 0;
    for (vector<int>::const_iterator i=dirrichBC.begin(); i!=dirrichBC.end(); ++i) if (*i>=0) blockedDoFNum ++;
    for (vector<int>::const_iterator i=neumannBC.begin(); i!=neumannBC.end(); ++i) if (*i>=0) loadedDoFNum ++;
}
...
vector<unsigned> BoundaryCondition::giveBlockedDoFs() const{
    ...
    unsigned k = 0;
    unsigned s = 0;
    for (vector<int>::const_iterator i=dirrichBC.begin(); i!=dirrichBC.end(); ++i, k++) if (*i>=0) {blocked[s] = node->giveStartingDoF()+k; s++;};
    return blocked;
}
...
vector<unsigned> BoundaryCondition::giveLoadedDoFs() const{
    ...
    unsigned k = 0;
    unsigned s = 0;
    for (vector<int>::const_iterator i=neumannBC.begin(); i!=neumannBC.end(); ++i, k++) if (*i>=0) {loaded[s] = node->giveStartingDoF()+k; s++;};
    return loaded;
}
...
vector<unsigned> BoundaryCondition::giveBlockedFunctions() const{
    ...
    unsigned k = 0;
    unsigned s = 0;
    for (vector<int>::const_iterator i=dirrichBC.begin(); i!=dirrichBC.end(); ++i, k++) if (*i>=0) {blocked[s] = dirrichBC[k]; s++;};
    return blocked;
}
...
vector<unsigned> BoundaryCondition::giveLoadedFunctions() const{
    ...
    unsigned k = 0;
    unsigned s = 0;
    for (vector<int>::const_iterator i=neumannBC.begin(); i!=neumannBC.end(); ++i, k++) if (*i>=0) {loaded[s] = neumannBC[k]; s++;};
    return loaded;
}
...
BCContainer::~BCContainer(){
    for(vector<BoundaryCondition*>::iterator bc=BC.begin(); bc!=BC.end(); ++bc) delete *bc;    
}
