...
/////////////////////////////////./////////////////////////
void DiscreteTransportRVEMaterialStatus :: generateEigenStrainLoads() { 
    ...
    if ( !macromat->addMacroComponents() ) {
        ...
        y.resize(2, 0);              
        for ( unsigned i = 0; i < elems->giveSize(); i++ ) {
            if (!elems->giveElement(i)->doesTransport()) continue;
...
//////////////////////////////////////////////////////////
void DiscreteTransportRVEMaterialStatus :: computeStressPrecomputed(double timeStep) {
    ( void ) timeStep;
...
/////////////////////////////////./////////////////////////
void DiscreteTransportRVEMaterialStatus :: computeStress(double timeStep) {
    ( void ) timeStep;
    //precomputed material
    if ( is_precomputed ) {
        computeStressPrecomputed(timeStep);
    }   
...
/////////////////////////////////./////////////////////////
void DiscreteTransportRVEMaterialStatus :: computeStressWithFrozenIntVars(double timeStep) {
    ( void ) timeStep;
    computeConstitutiveStrain();        
    transformStrain();
...
Matrix DiscreteTransportRVEMaterialStatus :: giveStiffnessTensorLocalExact(string type) {
    ...
    Vector true_temp_strain = temp_strain_total;
    initializeStressAndStrainVector();    
    double factor = 1e-10;
    for ( unsigned i = 0; i < dimension; i++ ) {
        temp_strain_total [ i ] = factor;         
        computeStress(-1);  
        temp_strain_total [ i ] = 0.;
...
void DiscreteTransportRVEMaterialStatus :: setFromPrecomputedToFullModel() {
    ...
    Vector prev_temp_strain = temp_strain;     
...
void DiscreteTransportRVEMaterialStatus :: init() {
    ...
    if ( macromaterial->givePrecomputedCapacity() < 0 ) {
        ...
        unsigned q = 0;
        while ( !RVE->giveElements()->giveElement(q)->doesTransport() ) {
            q++;
        }
        ...
        if ( macromaterial->shouldStartFromPrecomputed() && stiff.size() == 0 ) {
            ...
            if ( macromaterial->isElasticSolutionVoigt() ) {
                ...
                Keff = giveStiffnessTensorLocalExact("elastic");
            }
...
//////////////////////////////////////////////////////////
void DiscreteTransportRVEMaterial :: readFromLine(istringstream &iss) {
...
//////////////////////////////////////////////////////////
void DiscreteMechanicalRVEMaterialStatus :: computeConstitutiveStrain() {
    DiscreteMechanicalRVEMaterial *macromaterial = static_cast< DiscreteMechanicalRVEMaterial * >( mat );
    if ( eigenstrain.size() > 0 ) {
        ...
        temp_strain = macromaterial->strainToCosserat(temp_strain_total) - macromaterial->strainToCosserat(eigenstrain);
    } else if ( temp_strain_total.size() > 0 ){
        temp_strain = macromaterial->strainToCosserat(temp_strain_total);
    }
}


/////////////////////////////////./////////////////////////
void DiscreteMechanicalRVEMaterialStatus :: computeStressWithFrozenIntVars(double timeStep) {
    ( void ) timeStep;
    DiscreteMechanicalRVEMaterial *macromaterial = static_cast< DiscreteMechanicalRVEMaterial * >( mat );
    computeConstitutiveStrain();    
    transformStrain();
...
/////////////////////////////////./////////////////////////
void DiscreteMechanicalRVEMaterialStatus :: computeStress(double timeStep) {
     ...
     *  ssnl->addToEigenErrorValue_rPF(0, X);
     *
     ...
     *      }
     *      ssnl->addToEigenErrorValue_fext(0, 2.*X);
     *  }
     *  cout << "X "  << X << endl;
     *  ssnl->addToEigenErrorValue_Wext(0, 0);
     * }
     */
...
Matrix DiscreteMechanicalRVEMaterialStatus :: giveStiffnessTensorLocal(string type) const {
    ...
    if ( is_precomputed || macromat->hasPrecomputedTensorsStored() ) {
        ...
        unsigned strain_size = macromat->giveInternalStrainSize();
        unsigned dim = mat->giveDimension();
        Matrix Keff = Matrix :: Zero(strain_size, strain_size);
        vector< vector< Matrix > > *projectors = macromat->giveProjectors();
        ElementContainer *elems = RVE->giveElements();
...
Matrix DiscreteMechanicalRVEMaterialStatus :: giveStiffnessTensorLocalExact(string type) {
    ...
    //compute exactly
    DiscreteMechanicalRVEMaterial *macromaterial = static_cast< DiscreteMechanicalRVEMaterial * >( mat );    
    cout << "PrecomputingiveInternalStrainSizeg primary fields for mechanical RVE" << endl;
...
void DiscreteMechanicalRVEMaterialStatus :: setFromPrecomputedToFullModel() {
    ...
    cout << "*** initialization of mechanical RVE in element " << element->giveID() << " and IP " << idx << " ***" << endl;
    Vector prev_temp_strain = temp_strain;     
    RVEMaterialStatus :: init(); 
    temp_strain = prev_temp_strain; 
}
...
DiscreteMechanicalRVEMaterial :: DiscreteMechanicalRVEMaterial(unsigned dimension) : DiscreteTransportRVEMaterial(dimension) {
    ...
    internal_strainsize = dim * dim;
    internal_strainsize += ( dim == 3 ) ? dim * dim : dim; //in 2D only vector
    strainsize = internal_strainsize;
...
//////////////////////////////////////////////////////////
void DiscreteMechanicalRVEMaterial :: setConversionFromCauchy(bool convert){
    convert_from_cauchy=convert;
...
Vector DiscreteMechanicalRVEMaterial :: strainToCosserat(Vector strain) {
    if ( convert_from_cauchy && CauchyToCosseratMatrix.cols()==strain.size()) {
        return CauchyToCosseratMatrix * strain;
    }
    if (giveInternalStrainSize()!=strain.size()){
        cerr << "Error in DiscreteMechanicalRVEMaterial: expected strain tensor of size " << CauchyToCosseratMatrix.rows() << " but size " << strain.size() << " provided" << endl;
        exit(1);
    }
    return strain;
}
...
//////////////////////////////////////////////////////////
void DiscreteCoupledRVEMaterialStatus ::  computeStress(double timeStep) {
    ...
    unsigned sizeM = dmm->giveStrainSize();
    unsigned sizeT = trspRVEstat->giveMaterial()->giveStrainSize();
    Vector strainM = Vector :: Zero(sizeM);
    Vector strainT = Vector :: Zero(sizeT);
    unsigned i;
    for ( i = 0; i < sizeM; i++ ) {
        strainM [ i ] = temp_strain [ i ];
        mechRVEstat->setTotalTempStrain(strainM);
    }
    for ( i = 0; i < sizeT; i++ ) {
        strainT [ i ] = temp_strain [ i + sizeM ];
        trspRVEstat->setTotalTempStrain(strainT);
    }

    //solve mechanics
    mechRVEstat->computeStress(timeStep);
    Vector stressM = mechRVEstat->giveTempStress();
    ...
    trspRVEstat->computeStress(timeStep);
    Vector stressT = trspRVEstat->giveTempStress();
...
//////////////////////////////////////////////////////////
void DiscreteCoupledRVEMaterialStatus ::  computeStressWithFrozenIntVars(double timeStep) {
    ( void ) timeStep;
    computeConstitutiveStrain();    
    DiscreteCoupledRVEMaterial *dcm = static_cast< DiscreteCoupledRVEMaterial * >( mat );
    unsigned ndim = dcm->giveDimension();
    temp_stress = giveStiffnessTensor("secant") * temp_strain;
    updateRateVariables(timeStep);
    double mechBiot = -temp_pressure *dcm->giveBiotCoefficient();  //take pressure stored in element for integration point IDX
    for ( unsigned i = 0; i < ndim; i++ ) {
        temp_stress [ i ] += mechBiot;
...
//////////////////////////////////////////////////////////
void DiscreteCoupledRVEMaterialStatus ::  addToEigenStrain(Vector &x) {
    DiscreteMechanicalRVEMaterial *dmm = static_cast< DiscreteMechanicalRVEMaterial * >( mechRVEstat->giveMaterial() );
    unsigned sizeM = dmm->giveStrainSize();
    unsigned sizeT = trspRVEstat->giveMaterial()->giveStrainSize();
    ...
    mechRVEstat->addToEigenStrain(eigenstrainM);
    trspRVEstat->addToEigenStrain(eigenstrainT);
}
