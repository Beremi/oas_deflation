...
void NodeContainer::establishDoFArray(){
    ...
    for(vector<unsigned>::iterator d=DoFid.begin(); d!=DoFid.end(); ++d,id++){
        if (id==a[k].first){
            *d = freeDoFs+k;
            blockedDoFid[a[k].second]=k;
            k++;
        }
        else *d = id-k;
    }
    for(int i=0; i<loaded.size(); i++)  loadedDoFid[i]=DoFid[loaded[i]];
...
void NodeContainer::addRHS_nodalLoad(Vector &RHS, double time) const{
    ...
    for(int k=0; k<loaded.size(); k++) RHS[loadedDoFid[k]] += loaded[k];  
}

//////////////////////////////////////////////////////////
void NodeContainer::updateDirrichletBC(Vector &bc, double time) const {
    vector<double> blocked = BC->giveBlockedDoFValues(time);
    for(int k=0; k<blocked.size(); k++) bc[blockedDoFid[k]] = blocked[k];
}
