...
double MarsMaterialStatus::giveValue(string code)const{
    if(code.compare("damage")==0) return temp_damage;
    else return DisMechMaterialStatus::giveValue(code);
}
...
void MarsMaterialStatus::computeOmega0(){
    ...
    while (fabs(err) > 1e-5) {
        if (err * o1 < 0.) o1 = omega0;
        else o2 = omega0;
        omega0 = (o1 + o2) / 2.;
        err = giveS0tension(omega0) - giveS0compression(omega0);
    }
    if (omega0 > 0.0) omega0 -= 2. * M_PI;
    if (omega0 < -0.5 * M_PI) omega0 = -M_PI - omega0;
}
...
void MarsMaterialStatus::computeDamage(Vector strain){
    ...
    double epsT;
    if(strain.size()==2) epsT = abs(strain[1]);  //2D
    else epsT = sqrt(pow(strain[1],2) + pow(strain[2],2)); //3D        
    double epsEQ = sqrt(pow(epsN,2) + m->giveAlpha()*pow(epsT,2)); //equivalent strain

    if (epsEQ > 0 && damage < 1.0) {
        double omega, S0, chi, K0, strEQ;
        if (epsT > 0) omega = atan(epsN / (sqrt(m->giveAlpha()) * epsT));
        else if (epsN>0) omega = 0.5 * M_PI;
        else omega = -0.5 * M_PI;
        ...
        if (omega < omega0) { //compression
                ...
                if (omega < 0.0) chi = epsEQ * omega / omega0 + emax * (1. - omega / omega0);
                else chi = emax;            
        }
        if (chi - S0 / m->giveE0() > 0) strEQ = S0 * exp(K0 / S0 * (chi - S0 / m->giveE0()));
        else strEQ = S0;
...
void MarsMaterial::readFromLine(istringstream &iss){
    ...
    if (not bft) {cerr << name << ": material parameter 'ft' was not specified" << endl; exit(0);};
    if (not bGt) {cerr << name << ": material parameter 'Gt' was not specified" << endl; exit(0);};
};
