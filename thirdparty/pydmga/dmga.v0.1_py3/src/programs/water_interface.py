    ...
    while(len(queue) > 0):
        ...
        for side in cell.sides:
            ...
            if (neighbour < solvent_from):
                ...
                # add new polygon to this part
                surface_by_part[-1].append(polygon)
            else:
...
for i, part in enumerate(surface_by_part):
    ...
    for polygon in part:
        if (pa_id + len(polygon) >= 9999):
            ...
            file_count += 1
            f = open(
                "/home/robson/{0}_{1:03d}_{2:03d}.pdb".format(filename, i, file_count), "w")
            f.write(cryst_line)
            pa_id = 0
